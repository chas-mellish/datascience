...
olives_pca_variance <- olives_pca$sdev**2
olives_pca_variance / sum(olives_pca_variance) -> variation_explained

tibble(variation_explained) %>%
  mutate(PC = paste0("PC", 1:8)) -> final_variation

  ggplot(final_variation, aes(x = PC, y = variation_explained)) +
  geom_col(color = "navyblue",
           fill = "hotpink",
           alpha = 0.75) +
  geom_text(aes(x = PC,
            y = variation_explained + 0.02,
            label = round(variation_explained, 5))) +
  labs(x = "Principal Components",
       y = "Percent of Variation in Data Explained")
...
```
+ **Bonus Explanation**
  + The *region* for each olive does not cluster together here unforunately.
  
  + Though the bulk of certain clusters belong to a certain regions, most of *Cluster 2* residing in *Southern Italy* for example, there is no clear distinction between data clusters and the regions examined.
