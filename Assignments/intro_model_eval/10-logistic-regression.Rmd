...
#FAILURE FIRST (0), SUCCESS SECOND (1)
...
## Including [[1]] displays only the value, not the supplemental message
model1_roc$auc[[1]]
model2_roc$auc[[1]]
model3_roc$auc[[1]]

## Model has the best AUC making it the best model out of the three
```
...
bind_rows(model1_tibble, model2_tibble, model3_tibble) %>%
  ...
  ggplot(aes(x = FPR, y = TPR, 
             color = fct_reorder(model, auc, .desc = TRUE))) +
...
1. Which model has the highest AUC value, and what is its AUC? Given that AUC can range 0.5-1, do you believe this is highly accurate model?
  ...
  + Model 1 has the **highest** AUC value of 0.839, this makes it a highly accurate model as it is fairly close to 1.
...
2. Compare models 2 and 3. What are the AUC values for each? Does including `skin` in the model seem to improve the model's performance?
  ...
  + Model 3 **(AUC = 0.8198)** has a higher AUC value than Model 2 **(AUC = 0.8186)** making it the better of the two. The inclusion of *skin* was not a favorable addition to this model as it did not increase the AUC by a signifiant amount (~ 0.0012).
...
tibble(x = train_model$linear.predictors,
  ...
  ggplot(aes(x = x, y = y)) +
  geom_line() + 
  geom_point(aes(color = diabetic), size = 0.5) +
  theme(legend.position = "bottom") +
  labs(x = "Linear Predictors",
       y = "Probability Diabetic",
       title = "Training Model",
       color = "Diabetic") -> train_plot
...
bind_rows(train_data, test_data) %>%
  #Combines data
  ggplot(aes(x = FPR, y = TPR)) +
  geom_line(aes(size = model, color = model)) +
  scale_color_manual(values = c("forestgreen", "gold")) +
  scale_size_manual(values = c(1,1)) +
  #Resizing and coloring to make the curves visible
  labs(title = "ROC Curves of Training & Testing Data",
       color = "Model") +
  theme_minimal()
...
+ This model performs well when considering the split, the resulting AUC of both the training split (~85%) and the testing split (~83%) are high but not clearly over-fitted.
...
+ Accuracy = 0.715 or ~72%
+ False Positive Rate = 0.0246 or ~2.5%
...
tibble(prob_diabetic = model1$fitted.values,
  ...
  pivot_wider(names_from = classif,
              values_from = n) -> final_p4
final_p4

## Calculating Accuracy = 0.715
## High threshold leads to moderate accuracy
final_p4 %>%
  mutate(Accuracy = (TP + TN) / (TP + TN + FP + FN))

## Calculating FPR = 0.0246
final_p4 %>%
  mutate(FPR = FP / (TN + FP))

## Calculating PPV = 0.838
final_p4 %>%
  mutate(TPR = TP / (TP + FP))
...
1. At a threshold of 80%, does the model predict that Woman 1 is diabetic? Answer yes or no!
  ...
  + With a threshold of **80%** Woman 1 is *not* diabetic, her response was ~11%.
  
2. At a threshold of 80%, does the model predict that Woman 2 is diabetic? Answer yes or no!

  + With a threshold of **80%** Woman 2 is *not* diabetic, her response was ~79.8%.