...
knitr::opts_chunk$set(echo = TRUE, fig.width=4, fig.height=3, fig.align = "center", tidy.opts=list(width.cutoff=50),tidy=TRUE)
library(tidyverse)
library(patchwork)
library(prettydoc)
library(formatR) #Installed this to apply wrapping for codechunks.
...
ggplot(sparrows, aes(x = Survival)) +
  geom_bar(width = 0.7, fill = "burlywood4") +
  labs(y = "Count", x = "Survival") +
  theme_bw()
```
...
sparrows$Survival <- factor(sparrows$Survival, levels = c("Dead", "Alive")) ##Makes survival a factor rather than a character, Dead or Alive are now LEVELS.
print(levels(sparrows$Survival)) ##Shows the results of the function above.
sparrows$Age <- factor(sparrows$Age, levels = c("Young", "Adult"))
print(levels(sparrows$Age))
ggplot(sparrows, aes(x = Survival)) +
  geom_bar(width = 0.7, fill = "darkmagenta") +
  labs(y = "Count", x = "Survival") +
  theme_bw()
```
...
Make one of each type of barplot, and in the space below, answer whether Young or Adult sparrows were more likely to survive. Write an additional 1-2 sentences describing which visualization choice makes it easier to answer this question.
  

```{r survival-age-stacked}
stacked1 <- ggplot(sparrows, aes(x = Survival, fill = Age)) + #Defined this plot as a variable for the next question, same is done below.
  geom_bar(width = 0.7) + #Automatically applies the stacked barplot, no coding required.
  scale_fill_manual(values = c("lightcyan3", "darkorange4")) +
  labs(y = "Count", x = "Survival") +
  theme_bw()
print(stacked1)
```
...
dodged1 <- ggplot(sparrows, aes(x = Survival, fill = Age)) +
  geom_bar(width = 0.7, position = "dodge") + #Adjusting this position will separate the bars.
  scale_fill_manual(values = c("lavenderblush3", "hotpink4")) +
  labs(y = "Count", x = "Survival") +
  theme_bw()
print(dodged1)
```
...
While both visualization choices are easy to interpret I believe that the latter option, a dodged barplot, is easier to read. With a quick once-over I can tell that the adult sparrows are morelikely to survive.
...
```{r survival-sex-patch1, fig.width = 6}
## Add the plots for horizontal arrangement
stacked1 + dodged1 

#Needed to define plots as a varaible first in the previous code for Question 3, then you can patch them together.
```
...
ggplot(sparrows, aes(x = Skull_Width, fill = Survival)) + #In the iris set the titles are separated by periods while in sparrows they use underscores.
...
<answer>
<!-- your answer goes in here. DO NOT DELETE THE ANSWER TAGS -->
It really does not look like there is much difference between alive or dead sparrows and their corresponding skull widths.    
...
```{r survival-skull-density-facet, fig.width = 5}
ggplot(sparrows, aes(x = Skull_Width, fill = Survival)) +
  geom_density(alpha = 0.4) +
  scale_fill_manual(values = c("coral3", "powderblue")) +
  theme_bw() +
  labs(x = "Skull Width", y = "Density") +
  facet_wrap(~Sex)
```
...
<answer>
<!-- your answer goes in here. DO NOT DELETE THE ANSWER TAGS -->
After facteting these images it appears that skull width DOES have an influence on survival when sex is also considered. Male sparrows with thinner beaks have a higher survival rate than those with thicker beaks. Additionally, females do not show this trend.
</answer>
...
```{r weight-sex-age-jitter, fig.height = 4.5}
ggplot(sparrows, aes(x = Sex, y = Weight, color = Age)) +
  geom_jitter(width = 0.3, alpha = 0.7) +
  scale_color_manual(values = c("palevioletred3", "paleturquoise4")) +
  theme_bw() +
  theme(legend.position = "bottom") #Just a reminder this is how to move the legend to the bottom of the figure.
```
...
ggplot(sparrows, aes(x = Sex, y = Weight, color = Length)) +
  ...
  scale_color_gradient(low = "#86bf9d", high = "#144a2a") + 
    theme_bw()
#I used the ColorPicker on Google to find these numbers, please don't think I stole someone else's code!!
```
...
```{r sternum-facet-histogram, fig.width = 5, fig.height = 4}
ggplot(sparrows, aes(x = Sternum_Length)) +
  geom_histogram(bins = 6, color = "black", aes(fill = Age)) +
  labs(x = "Sternum Length", y = "Count") +
  facet_grid(Sex~Age) + 
  theme_bw()
#Turns out I can use the actual subjects rather than using the variables 'x' and 'y' to define Sex or Age, HUGE goof moment.
```
...
```{r skull-scatter, fig.height = 3, fig.width = 4}
q10 <- ggplot(sparrows, aes(x = Skull_Length, y = Skull_Width)) +
   geom_point(aes(color = Sex)) +
   scale_color_manual(values = c("khaki3", "deepskyblue3")) +
   theme_bw() +
   labs(x = "Skull Length", y = "Skull Width")
print (q10) ## The print() function is needed bc I defined the plot as q10.
```


<answer>
<!-- your answer goes in here. DO NOT DELETE THE ANSWER TAGS -->
It appears that there is a positive relationship between skull length and skull width, as length increases so does width. Male and female skulls appear to follow the trend similarly; no difference between these relationships and sexes.    
...
```{r skull-scatter-title, fig.height = 3, fig.width = 4}
q10 + labs(title = "Skull Size by Sparrow Sex")
```
...
<answer>
<!-- your answer goes in here. DO NOT DELETE THE ANSWER TAGS -->
Each of the regression lines were different in these plots as the location of the aes(color = Sex) changes, when the code is within the the geom_point() there is a single trendline for the entire dataset. When the code is placed within the ggplot() there are two trendlines per sex. Although these trends vary slightly in their slope it doesn't appear there is a significant difference in these regression lines.
...
ggplot(sparrows, aes(x = Skull_Length, y = Skull_Width, color = Sex)) + 
  geom_point() + 
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("darkorchid", "cyan3")) +
  theme_bw() +
  labs(x = "Skull Length", y = "Skull Width")
```
...
<answer>
<!-- your answer goes in here. DO NOT DELETE THE ANSWER TAGS -->
To correct this mistake the data mapping for 'x' and 'y' on this geom needed to be specified in the original ggplot line of code since the geom derives its data from this function.
...
ggplot(sparrows, aes(x = Skull_Length, y = Skull_Width, color = Sex)) + 
  geom_text(aes(label = "B")) + ## Use the label function to assign the desired letter or subect.
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("goldenrod2", "mediumpurple4")) +
  theme_bw() +
  labs(x = "Skull Length", y = "Skull Width")
```
...
ggplot(sparrows, aes(x = Skull_Length, y = Skull_Width, color = Sex)) + 
  ...
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("palevioletred3", "navyblue")) +
  theme_bw() +
  labs(x = "Skull Length", y = "Skull Width")
```
...
Practice specifying _two new types of shapes_, one that CAN NOT be filled and one that CAN BE filled, in a scatter plot of Skull Width vs Skull Length below. Specify your shape like this: `geom_point(shape=...)` where `...` is the number shape.

```{r skull-scatter-point-color-and-fill, fig.height = 5, fig.width = 6}
ggplot(sparrows, aes(x = Skull_Length, y = Skull_Width, color = Sex, fill = Sex)) + 
  geom_point(shape = 23) + 
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("olivedrab", "royalblue3")) +
  scale_fill_manual(values = c("seashell3", "darkgrey")) +
  theme_bw() +
  labs(x = "Skull Length", y = "Skull Width")
```
    
    
```{r skull-scatter-point-color-onlu, fig.height = 5, fig.width = 6}
### Scatterplot with type of point NOT 21,22,23,24,25. Specify a color (not aesthetic - just straight up color the points)
ggplot(sparrows, aes(x = Skull_Length, y = Skull_Width, color = Sex)) + 
  geom_point(shape = 8) + 
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("tomato", "steelblue4")) +
  theme_bw() +
  labs(x = "Skull Length", y = "Skull Width")
```
