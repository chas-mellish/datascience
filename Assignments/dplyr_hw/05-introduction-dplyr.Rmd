...
**This assignment must be submitted as an RMarkdown document on Blackboard by _Thursday February 20th at 10:00 am_.** 
...
```{r}
CO2_tibble %>%
  filter(Type == "Mississippi") #Shows only the rows containing Mississippi, the '==' create a TRUE/FALSE argument.
```
...
```{r}
CO2_tibble %>%
  select(- uptake, - conc) #The '-' symbol needs to be applied to each of these columns to remove them, otherwise the select() will just show that specified column.
```
...
```{r}
CO2_tibble %>%
  mutate(conc_cLL = conc/10) # Will still work if you place conc_cLL in "" but NOT conc/10.
```
...
```{r}
CO2_tibble %>%
  mutate(conc_cLL = conc/10) %>% #This creates a unit conversion column.
...
```{r}
CO2_tibble %>%
  mutate(high_uptake = uptake>=38) #Only tells you which of the values is above (TRUE) 38 or below (FALSE) because we are looking for values equal to or greater than this number.
```
...
```{r}
CO2_tibble %>%
  mutate(mean_uptake = mean(uptake)) #Creates new column that contains the single average of all the uptake values.

CO2_tibble %>%
  summarize(mean(uptake)) #Condenses the average of the uptake values into one row with a single value.
```
...
#Defined the refined dataset as quebec, a new tibble used in Q12.
...
```{r}
quebec %>%
  group_by(Treatment) %>% #Defines how we are going to make these independent groups.
  summarize(mean(uptake))
```
...
quebec %>%
  group_by(Treatment) %>%
  filter(conc == 500) %>% #Focuses on the specified concentration.
  summarize(median(uptake))
```
...
quebec %>%
  filter(conc == 500) %>%
  tally(uptake <=25) #Counts the number of items under this criteria.
```
...
CO2_tibble %>%
  ...
  group_by(Type) %>%
  summarize(mean(uptake))
#Quebec has the higher average uptake in nonchilled plants that were exposed to a CO2 concentration of 1000mL/L.
```
...
### q14 code way 1
CO2_tibble %>%
  filter(Treatment == "chilled") -> chilly #Defines a new variable with the filtered information.

chilly %>% #We can pipe the created variable directly into ggplot.
  ggplot(aes(x = conc, y = uptake, group = Plant, color = Plant)) +
  geom_line() +
  geom_point() +
  facet_grid(~Type) +
  labs(x = "Concentration", y = "Uptake")
...
CO2_tibble %>%
  filter(conc == 175) %>%
  ggplot(aes(y = uptake, fill = Treatment)) +
...
#new_labels <- c("Nonchilled", "Chilled")   

#CO2_tibble %>%
  #mutate(Type_factor_custom_levels = factor(Type, levels = new_levels)) %>%
  #mutate(Treatment_factor_custom_labels = factor(Treatment, labels = new_labels))

```

```{r}
CO2_tibble %>%
  mutate(Type_factor_custom_levels = factor(Type, levels = c("Mississippi", "Quebec")), Treatment_factor_custom_labels = factor(Treatment, labels = c("Nonchilled", "Chilled"))) %>%
  select(-Type, -Treatment) -> question_16

question_16 %>%
  group_by(conc, Treatment_factor_custom_labels, Type_factor_custom_levels) %>%
  summarize(mean_uptake = mean(uptake)) %>%
  ggplot(aes(x = factor(conc), y = mean_uptake, fill = Treatment_factor_custom_labels)) +
  geom_bar(stat="identity") +
  facet_grid(Type_factor_custom_levels ~ Treatment_factor_custom_labels) +
  labs(x = "CO2 Concentration", y = "Mean Uptake", fill = NULL) +
  theme_bw() +
