...
theme_set(theme_bw())
fav_fill <- "darkgoldenrod1"
fav_color <- "dodgerblue4"

path_to_data <- "dplyr_lab2/" #Prevents us from HARDCODING the path into variables.
```
...
low
#Use 'paste0' to attach two strings together without spaces.
```
...
low %>%
  mutate(tide = "low") -> low_newtidecol
low_newtidecol

high %>%
  mutate(tide = "high") -> high_newtidecol
high_newtidecol

bind_rows(low_newtidecol, high_newtidecol) -> algae_full
algae_full

write_csv(algae_full, paste0(path_to_data, "algae_full.csv"))
```
...
```{r}
## This code chunk should read in, save, print the datasets only
diet <- read_csv(paste0(path_to_data, "mammals_diet.csv"))
diet
...
### Answer (Q2):
+ Inner join only combined the common data between the two datasets by name & order.
+ Left join brought data in from the left originating from info and filled in NA for values not found in the `mammals.info`.
+ Right join brought data in from the right originating from info and filled in NA for values for diets which aren't present in `mammals.info`.
+ Anti join only combined data that is not common between both datasets, this presented only the unique species data from `mammals.diet`.
...
```{r}
## perform tasks here
inner_join(info, diet) %>%
  inner_join(dietfoods) %>%
  arrange(bodywt_kg)
```
...
drop_na(msleep) -> dropped_dataset
head(dropped_dataset)

msleep %>%
  drop_na(conservation) -> dropped_conservation
head(dropped_conservation)

msleep %>%
  drop_na(brainwt) -> dropped_brain
head(dropped_brain)

msleep %>%
  replace_na(list(brainwt = 0)) -> zero_brain
head(zero_brain)
```
...
msleep %>%
  summarize(avg_brainwt = mean(na.rm = TRUE, brainwt))
...
### Answer (Q4):
+ The average presented from `dropped_dataset` only applies the function `mean()` to rows that had **NO NA** values. Therefore, this is innaccurate because it removes animals with recorded brain weights but missing observations in other columns.

+ The average presented from `dropped_convseration` only applies the function `mean()` to rows that have NA values in the **conservation** column. Therefore, this is inaccurate for the same reason specified for `dropped_dataset`.

+ The average presented from `droppped_brain` applies to any brain weight observation with a recorded values, thus excluding NA values from this column **only**. This is one of the **CORRECT** averages out of the six collected as the only excluded values presented as NA in the `brainwt` column.

+ The average presented from `zero_brain` applies to the entire brain weight column, replacing all of the NA values with 0. Therefore, this is incorrect as all the values considered in the calculation weren't actually recorded. Thus, the sum is being divided by an inaccurate count making this average invalid.

+ The average presented from the `msleep` dataset applies to the entire brain weight column, including all of the NA values present here. The resulting average value is incorrect as NA is not a number, it is the absence of a number.

+ The average presented from the `msleep` dataset with the added `na.rm = TRUE` argument takes the average of the brain weight column after removing any NA value in this column only. This is one of the **CORRECT** averages out of the six collected as the only excluded values presented as NA in the `brainwt` column.
...
# wrangle
pima %>%
  mutate(glucose_status = 
           case_when(glucose <= 70 ~ "low",
                     glucose >70 & glucose <= 120 ~ "average",
                     glucose >120 & glucose <=160 ~ "above-average",
                     glucose >160 ~ "high")) -> glucose_scale
head(glucose_scale)
```
...
glucose_scale %>%
  ggplot(aes(x = diabetic, fill = glucose_status)) +
  geom_bar(position = "dodge") +
  labs(x = "Diabetic", y = "Count", fill = "Glucose Status")
```
...
#ifelse
pima %>%
  mutate(pregnant = if_else(npreg >0, "Yes", "No")) -> pregnancy
head(pregnancy)
```
...
## Pull function isolates a column from the dataset and creates an individual array. Using pull here an assertion is created, this checks to make sure I performed the task I wanted.

## An assertion is NOT ALWAYS a pull.

pregnancy %>%
  filter(pregnant == "No") %>%
  summarize(sumpreg = sum(npreg)) %>%
  pull(sumpreg)
...
pima %>%
  mutate(diabetic = 
         if_else(diabetic == "Yes", "TRUE", "FALSE")) -> diabetic_if
head(diabetic_if)
```
...
pima %>%
  mutate(diabetic = 
           case_when(diabetic == "Yes" ~ "TRUE",
                     diabetic == "No" ~ "FALSE")) -> diabetic_case
head(diabetic_case)
```
...
chicks <- read_csv(paste0(path_to_data, "feed_protein.csv"))
head(chicks)
```
...
bigchicks <- chickwts
chicks %>%
  rename(feed = feed_type) %>%
  full_join(by = "feed", bigchicks, chicks) -> final_chicks
head(final_chicks)

## HERE's HOW to think about this assertion:
# for the answer to be right, we know there needs to be three columns:
correct_ncol <- 3
if (ncol(final_chicks) != correct_ncol)
{
  stop("You have too many columns!")
}

if (ncol(final_chicks) == correct_ncol)
  {
    print("Good job, you have three columns!")
  }
```
...
final_chicks %>%
  group_by(feed, g_protein) %>%
  summarize(med_weight = median(weight))%>%
  ggplot(aes(x = med_weight, y = g_protein)) +
  geom_point(aes(color = feed)) +
  geom_text(aes(label = feed, color = feed)) +
  labs(x = "Median Weight", y = "Grams of Protein") +
...
final_chicks %>%
  ...
  ggplot(aes(x = med_weight, y = g_protein)) +
  geom_point(aes(color = feed)) +
  geom_text_repel(aes(label = feed, color = feed)) +
  labs(x = "Median Weight", y = "Grams of Protein") +
  theme(legend.position = "NONE")
```
...
flights %>%
  ...
  filter(dest == "LAX") %>%
  select(tailnum, month, air_time, dest) -> summertime
head(summertime)

planes %>%
  select(tailnum, year) -> manu_info
head(manu_info)

inner_join(summertime, manu_info) -> joined_flights
head(joined_flights)

joined_flights %>%
  drop_na() %>%
  group_by(month, tailnum, year) %>%
  summarize(avg_airtime = mean(air_time)) -> question_7
head(question_7)
...
question_7 %>%
  ggplot(aes(x = avg_airtime, y = year)) +
  geom_point(aes(color = month)) +
  scale_color_gradient(low = "#edc15a", high = "#f88379") +
  facet_grid(~ month) +
  labs(x = "Average Airtime", y = "Year") +
  theme(legend.position = "NONE",
...
### emoji plot
question_7 %>%
  ggplot(aes(x = avg_airtime, y = year, color = month)) +
  geom_text(label = emoji('heart'), size = 3, replace = TRUE) +
  scale_color_gradient(low = "#e69ea9", high = "#b76e79") +
  facet_grid(~ month) +
  labs(x = "Average Airtime", y = "Year") +
  theme(legend.position = "NONE",
        axis.title = element_text(size = 10))
